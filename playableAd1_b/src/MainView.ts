...
class MainView extends eui.Component implements eui.UIComponent {
    ...
    private arr = [new egret.Point(210, 544),
    ...
    private playStartAni() {
        this.playBgm();
        egret.Tween.get(this.kImgMan).to({ x: 800 }, 600, egret.Ease.cubicOut).call(() => {
    ...
    private touchFirst() {
        ...
        egret.Tween.get(this.kImgFood1).to({ x: 228, y: 300 }, 400, egret.Ease.sineIn).call(() => {
            //TODO  播放进度条代码，传入回调
            this.playSmog(1000, 240, 350)
    ...
    private touchSecond() {
        ...
        this.clearMc();
        this.soil.close();
        egret.Tween.get(this.kImgFood1).to({ x: 436, y: 296 }, 400, egret.Ease.sineOut).call(() => {
            ...
            egret.Tween.get(this.kImgWoman).to({ x: 300 }, 400, egret.Ease.cubicOut).call(() => {
                ...
                this.setProgress(1000, () => {
                    egret.Tween.get(this.kImgFood2).to({ x: 884, y: 329, scaleX: 1, scaleY: 1 }, 400, egret.Ease.sineIn);
                    this.kImgArrow.touchEnabled = true;
                    this.playBubble(890, 380);
                    this.soundBubble();
                }, 900, 540)
    ...
    private touchFouth() {
        this.guide.clearHand();
        egret.Tween.get(this.kImgFood1).to({ x: 597, y: 121 }, 300, egret.Ease.sineOut).to({ scaleX: 0.85, scaleY: 0.85 }, 100).wait(100).call(() => {
    ...
    private onCompleteFood() {
        ...
        // if(this.kProgress.value != 0){
            this.clearMc();
        egret.Tween.get(this.kImgFood2).to({ x: 389, y: 172 }, 300, egret.Ease.sineOut).to({ scaleX: 0.7, scaleY: 0.7 }, 100).wait(100).call(() => {
    ...
    private async playSmog(t: number, x, y) {
        this.system = <particle.GravityParticleSystem>await ParticleUtil.instance.getParticleAtH5(this.kGrpAction, smJson, sm, -180, -180);
    ...
    private playEndAni() {
        ...
        egret.Tween.get(this.kLbEnd).wait(2000).call(() => {
