...
class GameMain extends eui.Component implements eui.UIComponent {
    ...
    //引导相关变量
    private guideArr = [new egret.Point(115, 208), new egret.Point(210, 260),
    ...
    */
    private async initUI() {
        ...
        //初始化人物动画
        this.hippies = <dragonBones.EgretArmatureDisplay>await Dragon.createArmature(50, 110, 'hippies', this.kGrpPeople2, hippies_ske, hippies_tex, hippies_tex_d);
    ...
    private async playStartAni() {
        ...
        this.drumStickArr3.forEach((item => {
            this.playFei(0, item.x - 25, item.y);
            this.playSmog(0, item.x + 25, item.y);
        }));
        //234,500,800
        this.producePeople();
    }
    ...
    */
    ...
    private clickSoil() {
        ...
        arr.forEach(item => { item.x = item.x + 10;});
        this.drumStickArr3.forEach((item) => {
            egret.Tween.get(item).to({ scaleX: 0, scaleY: 0 }, 300);
        });

        let a = 0;
        let scaleArr = [1, 0.8, 0.7];
    ...
    private async conactDrumstick(flag: boolean = false) {
        ...
        tim.addEventListener(egret.TimerEvent.TIMER, async () => {
            ...
            let system = <particle.ParticleSystem>await ParticleUtil.instance.getParticleAtH5(this.kGrpAction, coinJson, coin, arr[a].x, arr[a].y, 700);
