...
class MainView extends eui.Component implements eui.UIComponent {
    ...
    private kImgD4: eui.Image;
    private aunt:egret.MovieClip;
    private grampa:dragonBones.EgretArmatureDisplay;
    ...
    //引导相关变量
    private guideArr = [new egret.Point(450, 600), new egret.Point(450, 600), new egret.Point(450, 600),
    ...
    */
    ...
    private async playStartAni() {
        ...
        //281 697
        let arr = [this.kImgD1,this.kImgD2,this.kImgD3,this.kImgD4];
        arr.forEach(item=>{
            item.alpha = .5;
            egret.Tween.get(item).wait(1000).to({alpha:1},3000);
            item.source = main_cake5
            this.playSmog(0,item.x,item.y);
            this.playFei(0,item.x-60,item.y);
        });

        this.grampa = <dragonBones.EgretArmatureDisplay>await Dragon.createArmature(120,150,'gramma',this.kGrpPeople2,gramma_ske_j,gramma_tex,gramma_tex_j);
        this.grampa.scaleX = -1;
        this.grampa.animation.play('walk',1);


        egret.Tween.get(this.kGrpPeople2).to({ x: 697 }, 700).call(() => {
            this.grampa.animation.play('stand',0);
